...
**Zapret 2** — второй, пакетный обход DPI, встроенный в LimCore. Он снимает троттлинг и разблокирует сайты **без какой-либо VPN-подписки**, рассинхронизируя рукопожатие выбранных потоков прямо на сетевом уровне. LimCore включает движок [bol-van/zapret2](https://github.com/bol-van/zapret2) (`nfqws2`) и управляет им за вас.
...
| ✅ **Работает на обоих ядрах** | Реализован как помеченный direct-outbound; одинаково работает на sing-box-extended и sing-box-extended. |
...
3. Цепочка nft (`LimCore_zapret_queue`) ловит помеченные пакеты и отправляет первые 1–12 пакетов каждого TCP/UDP-соединения в **NFQUEUE**.
...
1. Откройте **Сервисы → LimCore → Настройки узлов** и вкладку **Zapret**.
2. Если `nfqws2` ещё не установлен, нажмите **Установить** — LimCore скачает подходящий пакет [zapret2-openwrt](https://github.com/1andrevich/zapret2-openwrt) для вашей архитектуры.
