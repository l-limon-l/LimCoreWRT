...
Есть четыре способа добавить узлы в LimCore. Все они — на **Настройки узлов → Узлы** (кнопки импорта) и **Настройки узлов → Подписки**.
...
LimCore полностью декодирует share-формат Amnezia `vpn://` (`base64url(qCompress(zlib JSON))`) прямо в браузере — без вспомогательных утилит. Он читает контейнер внутри и автоматически распознаёт **оба** типа конфигов Amnezia:
...
Содержимое подписок и share-ссылок очень часто закодировано в **base64** — это норма, а не редкий случай. LimCore декодирует base64 прозрачно:
...
На вкладке **Подписки** добавьте одну или несколько **ссылок подписок**. Роутер скачивает их и поддерживает список узлов в актуальном состоянии. LimCore автоматически распознаёт несколько форматов ответа:

- **Base64 / обычные списки share-ссылок** — классический формат подписки (по одной кодированной ссылке на строку).
- **sing-box JSON / singbox** — когда подписка отдаётся как JSON-конфиг sing-box/singbox. Нужный формат запрашивается через **User-Agent**, поэтому один и тот же URL может возвращать разным клиентам разные форматы.
