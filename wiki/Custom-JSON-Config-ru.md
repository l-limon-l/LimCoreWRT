...
**Пользовательский JSON** — режим маршрутизации, позволяющий полностью обойти UCI-конфигурацию LimCore и использовать «сырой» JSON-конфиг формата [sing-box](https://sing-box.sagernet.org) / sing-box-extended напрямую. Предназначен для опытных пользователей, которым нужен тонкий контроль, недоступный через веб-интерфейс.
...
Файл конфигурации размещается по пути `/etc/LimCore/singbox-c.json`.

---

## Структура конфига

Конфиг sing-box-extended — это JSON-объект. Основные секции верхнего уровня:
...
## sing-box-extended и sing-box

sing-box-extended — форк sing-box, в целом совместимый по формату конфига. Ключевые отличия:

- **Дополнительные протоколы** — sing-box-extended поддерживает протоколы, которых нет в upstream sing-box или которые появились в нём позже. Это AnyTLS, расширенные опции для некоторых существующих протоколов. Полный список — на странице [Поддерживаемые протоколы](Supported-Protocols-ru).
- **Расширения singbox** — некоторые дополнительные поля и опции специфичны для singbox. Они задокументированы в [руководстве singboxCli](https://singbox.com/app/singboxCli-guide/#run-config-or-subscription-link-in-singboxcli-with-singboxapp-settings).
- **Различия версий** — sing-box-extended может опережать или отставать от конкретного релиза sing-box. Если функция sing-box не работает — проверьте release notes sing-box-extended.

При написании конфигов отталкивайтесь от документации sing-box, обращаясь к руководству sing-box-extended там, где поведение отличается.
...
```json
{
  "log": {
    "disabled": false,
    "level": "warn",
    "output": "/var/run/LimCore/singbox-c.log",
    "timestamp": true
  },
  "dns": {
    "servers": [
...
- **Логи.** Если конфиг применился, но трафик не работает — проверьте лог ядра в **LimCore → Ядро и службы** или через SSH: `tail -f /var/run/LimCore/singbox-c.log`.
...
- **`default_mark`.** **Обязательное** поле в секции `route` для предотвращения петель маршрутизации tproxy. Без него собственный исходящий трафик sing-box-extended перехватывается правилами nftables и снова направляется в прокси. Значение должно совпадать с `self_mark` в `/etc/config/LimCore` (по умолчанию: `100`):
