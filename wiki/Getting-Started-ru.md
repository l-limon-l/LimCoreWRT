...
Эта страница проведёт вас от свежей установки до рабочего соединения. Предполагается, что LimCore уже установлен (установку пакетов см. в [README](../../blob/master/README_ru.md)).

> Всё ниже делается в веб-интерфейсе LuCI: **Сервисы → LimCore**.

---

## Шаг 1 — Установите ядро

Приложение LuCI — это только интерфейс; проксированием занимается отдельный бинарник **ядра**. Откройте **Ядро и службы → Управление ядром** и нажмите **Установить**.

LimCore работает на **sing-box-extended** — это AmneziaWG/WARP и самый широкий набор протоколов. Установщик сначала проверяет свободное место и сверяет загрузку, поэтому он скорее откажет, чем оставит ядро установленным наполовину. Подробно: **[Управление ядром](Core-Management-ru)**.
...
На странице **Настройки клиента** задайте **Основной узел** = добавленный узел — или выберите **URLTest**, чтобы LimCore автоматически выбирал самый быстрый доступный узел и переключался при сбоях.
...
- **Проверка соединения** — проверяет доступность тестовых сайтов и показывает **Активный узел**: sing-box не умеет сообщать исходящий IP, поэтому вместо сравнения адресов выводится узел, который выбран сейчас.
