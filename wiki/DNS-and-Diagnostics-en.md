...
Two related topics: how LimCore resolves names (and why that matters for leaks), and how to verify everything works.
...
The **Diagnostics** page (Services → LimCore → Diagnostics) runs live checks. Each card has its own **Check** button.

| Card | What it shows |
|------|---------------|
| **Connectivity** | Reachability of test sites. On sing-box-extended it also shows your **Direct IP** vs **Proxy IP** (the two should differ when traffic is actually proxied); on **sing-box-extended** it shows the live **Active Node** instead, because sing-box can't report the exit IP. |
| **Core & System** | Whether the core (sing-box-extended / sing-box-extended) and ByeDPI are installed, the active binary, version, whether they're running (with PID), and listening ports. Includes a **Restart Service** button. |
...
- **Direct IP == Proxy IP** (sing-box-extended) → traffic isn't being proxied (check main node / routing mode / access control). On sing-box-extended, confirm the **Active Node** row shows your chosen node instead.
