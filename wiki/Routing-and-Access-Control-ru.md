...
LimCore решает, **какой** трафик проксировать (режим маршрутизации + правила) и **каких устройств** это касается (контроль доступа). Эта страница — про оба аспекта.
...
| **Пользовательский JSON** | Конфиг sing-box-extended вручную — см. [Пользовательский JSON конфиг](Custom-JSON-Config-ru). |
...
- Используйте страницу [Диагностика](DNS-and-Diagnostics-ru), чтобы убедиться, что трафик реально проксируется — **IP провайдера против IP прокси** на sing-box-extended или строка **Активный узел** на sing-box-extended.
