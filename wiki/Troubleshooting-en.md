...
If `opkg` itself cannot reach the internet, download the `.ipk` or `.apk` file on a PC from the [Releases page](https://github.com/l-limon-l/LimCoreWRT/releases) and transfer it to the router with `scp`.

---

## No Internet Access When LimCore Is Enabled (mwan3 Conflict)

**Symptom:** Ping and DNS work, but HTTP/HTTPS traffic fails when LimCore is enabled. Stopping mwan3 restores connectivity.
...
**Fix:** Tell the OS to skip mwan3's routing logic for traffic already handled by LimCore (marked `0x64`).
...
> This fix deliberately uses `iptables` because **mwan3** operates at the iptables layer. On a fw4 / nftables system it still applies through the iptables-nft compatibility shim — it does not conflict with LimCore's nft chains.
