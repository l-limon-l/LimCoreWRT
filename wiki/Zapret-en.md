...
**Zapret 2** is a second, packet-level DPI-bypass built into LimCore. It un-throttles and unblocks sites **without any VPN subscription** by desyncing the handshake of selected flows directly on the wire. LimCore bundles the [bol-van/zapret2](https://github.com/bol-van/zapret2) engine (`nfqws2`) and manages it for you.
...
| ✅ **Works on sing-box-extended** | Implemented as a marked direct outbound; works identically on sing-box-extended. |
...
1. Go to **Services → LimCore → Node Settings** and open the **Zapret** tab.
2. If `nfqws2` is not installed yet, use the **Install** button — LimCore fetches the right [zapret2-openwrt](https://github.com/1andrevich/zapret2-openwrt) package for your architecture.
