...
There are four ways to get nodes into LimCore. All live on **Node Settings → Nodes** (import buttons) and **Node Settings → Subscriptions**.
...
LimCore fully decodes Amnezia's `vpn://` share format (`base64url(qCompress(zlib JSON))`) right in the browser — no helper tool needed. It reads the container inside and recognises **both** Amnezia config types automatically:
...
Subscription and share-link payloads are very often **base64**-encoded — this is the norm, not an edge case. LimCore decodes base64 transparently:
