...
Две связанные темы: как LimCore резолвит имена (и почему это важно для утечек) и как проверить, что всё работает.
...
Страница **Диагностика** (Сервисы → LimCore → Диагностика) запускает живые проверки. У каждой карточки своя кнопка **Проверить**.

| Карточка | Что показывает |
|----------|----------------|
| **Проверка соединения** | Доступность тестовых сайтов. На sing-box-extended также показывает ваш **IP провайдера** против **IP прокси** (при реальном проксировании они должны различаться); на **sing-box-extended** вместо этого показывает **Активный узел**, так как sing-box не может сообщить исходящий IP. |
| **Ядро и система** | Установлены ли ядро (sing-box-extended / sing-box-extended) и ByeDPI, активный бинарник, версия, запущены ли они (с PID) и слушаемые порты. Есть кнопка **Перезапустить службу**. |
...
- **IP провайдера == IP прокси** (sing-box-extended) → трафик не проксируется (проверьте основной узел / режим маршрутизации / контроль доступа). На sing-box-extended убедитесь, что в строке **Активный узел** показан выбранный узел.
