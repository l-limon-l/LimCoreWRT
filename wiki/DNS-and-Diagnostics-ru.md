...
|----------|----------------|
| **Проверка соединения** | Доступность тестовых сайтов и строка **Активный узел**: sing-box не может сообщить исходящий IP, поэтому показывается выбранный сейчас узел. |
| **Ядро и система** | Установлены ли ядро (sing-box-extended) и ByeDPI, активный бинарник, версия, запущены ли они (с PID) и слушаемые порты. Есть кнопка **Перезапустить службу**. |
...
- В строке **Активный узел** пусто или не тот узел → трафик не проксируется: проверьте основной узел, режим маршрутизации и контроль доступа.
