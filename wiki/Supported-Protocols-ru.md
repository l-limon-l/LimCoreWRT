...
LimCore работает на [sing-box-extended](https://github.com/shtorm-7/sing-box-extended) — форке [sing-box](https://sing-box.sagernet.org) с дополнительными протоколами и функциями, недоступными в upstream. Отображаемые в редакторе узлов протоколы зависят от **флагов сборки установленной версии ядра**. Ядро устанавливается и обновляется в разделе **Управление ядром** (Сервисы → LimCore → Ядро и службы).
...
sing-box-extended добавляет функции, которых нет в upstream sing-box:
...
### AnyTLS
TLS-протокол с мультиплексированием. Простой, эффективный и сложно поддающийся отпечатыванию. Хороший выбор, когда другие TLS-протоколы блокируются.
...
**XHTTP** — расширенный транспорт для VLESS, доступный в sing-box-extended, но не в upstream sing-box. Использует chunked HTTP-передачу через одиночное или мультиплексированное соединение, специально разработан для совместимости с CDN и устранения паттернов, характерных для не-браузерного трафика.
...
Эти типы узлов дают расширенные флаги сборки **sing-box-extended**; в upstream sing-box их нет.
...
### MieruTCP / MieruUDP
Антицензурный протокол, разработанный независимо от sing-box и добавленный в sing-box-extended. Использует полностью случайные паттерны трафика без идентифицируемых заголовков или рукопожатий — крайне сложен для обнаружения и классификации системами DPI.
...
## Ожидается в будущих версиях ядра
...
Выполните команду версии — она печатает список флагов сборки (Tags):
