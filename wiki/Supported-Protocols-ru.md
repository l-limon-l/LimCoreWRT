...
LimCore работает на выбор ядра: [sing-box-extended](https://github.com/singbox/sing-box-extended) (по умолчанию) или [sing-box-extended](https://github.com/shtorm-7/sing-box-extended) — оба форки [sing-box](https://sing-box.sagernet.org) с дополнительными протоколами и функциями, недоступными в upstream. Отображаемые в редакторе узлов протоколы зависят от того, **какое ядро вы установили** и с какими флагами оно собрано на вашем устройстве. Ядро выбирается и устанавливается в разделе **Управление ядром** (Сервисы → LimCore → Ядро и службы).
...
Оба ядра — форки sing-box, добавляющие функции, которых нет в upstream. Часть из них **только в sing-box-extended, часть — в **обоих** ядрах (sing-box-extended и sing-box-extended), отмечено по каждому пункту:

### Фрагментация TLS (`tls_fragment`) *(только sing-box-extended)*
...
*Источник: [How the TLS Trick works and its usage — singbox.com](https://singbox.com/manager/basic-concepts-and-troubleshooting/How-the-TLS-Trick-works-and-its-usage/#tls-fragment)*

### Транспорт XHTTP *(оба ядра)*
Современный HTTP-транспорт для VLESS, разработанный для совместимости с CDN и мультиплексирования. Отсутствует в upstream sing-box, но доступен в **обоих** ядрах — sing-box-extended и sing-box-extended. Подробнее — в разделе VLESS ниже.
...
### AnyTLS
TLS-протокол с мультиплексированием, разработанный командой singbox. Простой, эффективный и сложно поддающийся отпечатыванию. Хороший выбор, когда другие TLS-протоколы блокируются.
...
Эти типы узлов доступны, если вместо sing-box-extended установить **sing-box-extended** (ядро выбирается на странице **Управление ядром**). sing-box-extended их **не** поддерживает.
...
### MieruTCP / MieruUDP
Антицензурный протокол, разработанный независимо от sing-box и добавленный расширенными ядрами (sing-box-extended **и** sing-box-extended). Использует полностью случайные паттерны трафика без идентифицируемых заголовков или рукопожатий — крайне сложен для обнаружения и классификации системами DPI.
...
## Ожидается в будущих версиях sing-box-extended
...
```sh
sing-box-extended version    # если установлен sing-box-extended
