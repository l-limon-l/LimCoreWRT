...
> **Protocol matching needs sniffing.** Matching by `quic`, `tls`, `bittorrent`, etc. relies on the inbound sniffing the first packets — LimCore enables this on its inbounds, so it works out of the box.
...
|---|---|---|
| How | UI: Routing Nodes + Routing Rules | Raw sing-box / sing-box-extended JSON |
