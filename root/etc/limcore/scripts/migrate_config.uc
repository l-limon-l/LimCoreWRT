...
import { isEmpty, parseURL } from 'limcore';
...
/* The keyword filter for subscription nodes has been removed */
for (let opt in ['filter_nodes', 'filter_keywords'])
	if (uci.get(uciconfig, 'subscription', opt) != null)
		uci.delete(uciconfig, 'subscription', opt);
