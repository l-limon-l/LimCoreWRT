...
import {
	...
	removeBlankAttrs, validation, normalizeVlessEncryption, LC_DIR, RUN_DIR
} from 'limcore';

const ubus = connect();

/* const features = ubus.call('luci.limcore', 'singbox_get_features') || {}; */
...
/* Where a proxy rule actually sends its traffic. A rule outlives the nodes it names:
 * deleting a subscription removes its nodes, not the rules pointing at them. A pool whose
 * members are all gone becomes a urltest with no outbounds, which sing-box refuses with a
 * FATAL — taking the whole network down with it, since the firewall keeps steering traffic
 * into a core that is not there. A rule left without its node goes through the main node
 * instead, and the page still shows what it was set to so it can be repointed. */
function ru_rule_node(cfg) {
	const node = cfg.node;
	if (isEmpty(node) || node in ['main-out', 'byedpi-out', 'zapret-out'])
		return node;
	if (node === 'urltest')
		return length(filter(cfg.urltest_nodes || [], (k) => uci.get_all(uciconfig, k) != null)) ? node : 'main-out';
	return (uci.get_all(uciconfig, node) != null) ? node : 'main-out';
}
...
/* The main node can name a node that is gone: a subscription removed or fetched again
 * takes its nodes with it, and a URLTest pool can lose every member the same way. A
 * missing node used to fail the whole generation and a pool with no members made
 * sing-box FATAL at start. Either falls back to the first node there is, so the network
 * keeps a proxy; with no node at all it is 'nil', which skips generation cleanly. */
function live_node(k) {
	return !isEmpty(k) && uci.get(uciconfig, k) === 'node';
}
...
function usable_main(node, pool_option, missing) {
	if (isEmpty(node) || node in ['nil', 'same', 'byedpi-out', 'zapret-out'])
		return node;
	if (node === 'urltest') {
		if (length(filter(uci.get(uciconfig, ucimain, pool_option) || [], live_node)))
			return node;
	} else if (live_node(node))
		return node;

	warn(sprintf('limcore: node "%s" (or every node of its pool) no longer exists, using "%s" instead.\n', node, missing));
	return missing;
}

if (routing_mode !== 'custom') {
	main_node = usable_main(uci.get(uciconfig, ucimain, 'main_node') || 'nil', 'main_urltest_nodes',
		first_node() || 'nil');
	...
	main_udp_node = usable_main(uci.get(uciconfig, ucimain, 'main_udp_node') || 'nil', 'main_udp_urltest_nodes', 'same');
	...
	direct_domain_list = trim(readfile(LC_DIR + '/resources/direct_list.txt'));
	if (direct_domain_list)
		direct_domain_list = split(direct_domain_list, /[\r\n]/);

	proxy_domain_list = trim(readfile(LC_DIR + '/resources/proxy_list.txt'));
...
/* Must match DELAY_PORT in the rpcd backend, which is the only thing that dials it. */
const DELAY_PORT = 9094;
...
/* An auto-select group from a subscription (see apply_balancers in update_subscriptions.uc):
 * a node of type urltest naming, by label, other nodes of the same subscription. */
function group_members(sid) {
	const g = uci.get_all(uciconfig, sid);
	if (!g || g.type !== 'urltest')
		return [];
	let found = {};
	uci.foreach(uciconfig, ucinode, (n) => {
		if (n.grouphash === g.grouphash && n.type !== 'urltest' && !(n.label in found))
			found[n.label] = n['.name'];
	});
	return filter(map(g.group_members || [], (l) => found[l]), (m) => m != null);
}

function push_outbound(list, node) {
	/* A group becomes a URLTest over its members, members first and the group last: every
	 * caller renames the last thing pushed (to main-out, a rule's outbound, ...), and that
	 * has to be the group. A member already in the list is not pushed twice. A group with
	 * no member left blocks rather than failing the start or leaking traffic direct. */
	if (node.type === 'urltest') {
		let tags = [];
		for (let m in group_members(node['.name'])) {
			const mc = uci.get_all(uciconfig, m);
			if (!mc || mc.type in ['wireguard', 'amneziawg'])
				continue;
			const tag = 'cfg-' + m + '-out';
			if (!length(filter(list, (o) => o?.tag === tag)))
				push_outbound(list, mc);
			push(tags, tag);
		}
		push(list, length(tags)
			? { type: 'urltest', tag: 'cfg-' + node['.name'] + '-out', outbounds: tags,
			    interval: '180s', tolerance: URLTEST_TOLERANCE, idle_timeout: '1800s' }
			: { type: 'block', tag: 'cfg-' + node['.name'] + '-out' });
		return;
	}
 ...
 *
 * Usage: generate_client.uc probe [clash_api_port] [socks_port]
 */
if (ARGV[0] === 'probe') {
	...
	uci.foreach(uciconfig, ucinode, (n) => {
		if (n.type === 'urltest')
			push(probe_groups, n['.name']);
		else if (n.type in ['wireguard', 'amneziawg'])
			push(probe_endpoints, generate_endpoint(n));
		else
			push_outbound(probe_outbounds, n);
	});
	/* A group is measured as the pool it is: its own URLTest over the members already
	 * emitted above, so the figure is the member it would pick. */
	for (let g in probe_groups) {
		const tags = map(filter(group_members(g), (m) => !(uci.get(uciconfig, m, 'type') in ['wireguard', 'amneziawg'])),
			(m) => 'cfg-' + m + '-out');
		push(probe_outbounds, length(tags)
			? { type: 'urltest', tag: 'cfg-' + g + '-out', outbounds: tags, tolerance: URLTEST_TOLERANCE }
			: { type: 'block', tag: 'cfg-' + g + '-out' });
	}

	/* One SOCKS port that reaches every node, the node chosen by the proxy username. The
	 * Clash API's delay test is a cold one — every call dials the node afresh, so its
	 * figure is TCP + TLS/REALITY handshake + the request, two to three round trips where
	 * a phone client that reuses the connection reports one. The same node read 60 ms warm
	 * and 190-425 ms through the API, from the same router a second apart. Going through
	 * this port lets the caller keep the connection open and time the request that comes
	 * after the handshake, which is the delay traffic actually sees. One port rather than
	 * one per node, so two dozen nodes do not need two dozen free ports. */
	const probe_socks = strToInt(ARGV[2]);
	let probe_in = null, probe_rules = null;
	if (probe_socks) {
		let users = [];
		probe_rules = [];
		for (let o in [ ...probe_outbounds, ...probe_endpoints ]) {
			push(users, { username: o.tag, password: 'x' });
			push(probe_rules, { auth_user: [ o.tag ], outbound: o.tag });
		}
		probe_in = [ { type: 'mixed', tag: 'probe-in', listen: '127.0.0.1', listen_port: probe_socks, users } ];
	}

	push(probe_outbounds, { type: 'direct', tag: 'direct-out' });

	/* The ping mode "default" (as Happ calls it) times every request as in a fresh core
	 * session, where the node's server name has to be looked up again as well. */
	const fresh = !(uci.get(uciconfig, ucimain, 'ping_mode') in ['double', 'keepalive']);

	printf('%.J\n', removeBlankAttrs({
		log: { level: 'error', timestamp: true },
		dns: fresh ? { disable_cache: true } : null,
		inbounds: probe_in,
...
if (!isEmpty(main_node)) {
	if (routing_mode === 'proxy_banned_ru') {
		...
		uci.foreach(uciconfig, ucirurule, (cfg) => {
			...
			const node = ru_rule_node(cfg);
			if (isEmpty(node) || node in ['main-out', 'byedpi-out', 'zapret-out'])
				return;
			if (index(pinned_sources, cfg.source) < 0)
				push(pinned_sources, cfg.source);
		});
		const ru_domain_tag = (src) => (src === 'refilter') ? 'lc-ru-refilter-domain' : ('lc-ru-' + src);
		for (let src in pinned_sources) {
			const dns_tag = 'lc-ru-' + src + '-dns';
			push(config.dns.servers, {
				tag: dns_tag,
				domain_resolver: {
					server: 'russia-dns',
...
/* Loopback-only port that leads straight to main-out, whatever the routing mode says, so
 * the page can time the node in use over a kept connection — the Clash API's delay test
 * redials on every call and reports the handshake along with the delay. Routed by the
 * first rule below. */
if (!isEmpty(main_node))
	push(config.inbounds, {
		type: 'mixed',
...
if (!isEmpty(main_node)) {
	...
	for (let i in urltest_nodes) {
		...
		/* Already there as the member of a group used for UDP. */
...
/* Routing rules */
if (!isEmpty(main_node)) {
	/* Ahead of the sniff, the resolve and every list: delay-in exists to measure main-out
	 * and nothing else, and a rule list that sent gstatic direct would measure the ISP. */
	...
	if (is_selective_mode(routing_mode)) {
		 ...
		 * here keeps a stale section from resolving to an 'lc-ru-' tag with nothing after it. */
		uci.foreach(uciconfig, ucirurule, (cfg) => {
			if (cfg.enabled === '1' && !isEmpty(cfg.source)) push(ru_rules, { ...cfg, node: ru_rule_node(cfg) });
		});
		...
		for (let cfg in ru_rules) {
			...
			if (cfg.node === 'main-out' || isEmpty(cfg.node))
				...
				effective_outbound = 'lc-ru-' + cfg.source + '-out';
			...
			const rule_sets = (cfg.source === 'refilter')
				? ['lc-ru-refilter-domain', 'lc-ru-refilter-ip']
				: (cfg.source === 'spotify')
			...
			if (cfg.source === 'refilter') {
				if (!has_ruleset('lc-ru-refilter-domain'))
					push(config.route.rule_set, {
						type: 'remote',
						tag: 'lc-ru-refilter-domain',
						format: 'binary',
						url: 'https://github.com/1andrevich/Re-filter-lists/releases/latest/download/ruleset-domain-refilter_domains.srs',
						download_detour: ruleset_detour,
						update_interval: '1d'
					});
				if (!has_ruleset('lc-ru-refilter-ip'))
					push(config.route.rule_set, {
						type: 'remote',
						tag: 'lc-ru-refilter-ip',
						format: 'binary',
				...
				if (!has_ruleset('lc-ru-spotify-ip'))
					push(config.route.rule_set, {
						type: 'remote',
						tag: 'lc-ru-spotify-ip',
						format: 'binary',
				 ...
				 * its shut-down games. */
				if (!has_ruleset('lc-ru-epicgames'))
					push(config.route.rule_set, {
						type: 'remote',
						tag: 'lc-ru-epicgames',
						format: 'binary',
				...
				if (!has_ruleset('lc-ru-google_ai'))
					push(config.route.rule_set, {
						type: 'remote',
						tag: 'lc-ru-google_ai',
						format: 'binary',
				...
				if (!has_ruleset('lc-ru-' + cfg.source))
					push(config.route.rule_set, {
						type: 'remote',
						tag: 'lc-ru-' + cfg.source,
						format: 'binary',
