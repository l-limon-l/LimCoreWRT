...
# A node whose server does not even accept a connection cannot be measured, and finding
# that out through a temporary core costs up to eleven seconds of the sweep; a bare TCP
# connect settles it in three and the sweep moves on to the next node. Only the TCP
# transports can be asked this way: the UDP ones (Hysteria, TUIC, WireGuard) have no
# connection to open, and those go straight to the core as before.
node_reachable() {
	case "$(uci -q get "limcore.$1.type")" in
		hysteria|hysteria2|tuic|wireguard|amneziawg|direct) return 0 ;;
	esac
	host=$(uci -q get "limcore.$1.address")
	port=$(uci -q get "limcore.$1.port")
	[ -n "$host" ] && [ -n "$port" ] || return 0
	case "$host" in *:*) host="[$host]" ;; esac

	# Any answer at all proves the handshake; only whether one happened is read, not
	# what the server made of a plain HTTP request on its port.
	t=$(curl -s -o /dev/null --connect-timeout 3 --max-time 4 -w '%{time_connect}' \
...
run_one() {
	...
	flush core
	if ! node_reachable "$SECTION"; then
		flush error "the server does not accept connections"
		commit
		return 0
	fi
