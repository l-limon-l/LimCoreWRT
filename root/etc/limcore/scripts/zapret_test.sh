...
# the live limcore queue (qnum 200) or other traffic.
#
...
cleanup() {
	...
	for hnd in $(nft -a list chain inet fw4 limcore_output_redir 2>/dev/null | grep "$CMT" | grep -oE 'handle [0-9]+' | grep -oE '[0-9]+'); do
		nft delete rule inet fw4 limcore_output_redir handle "$hnd" 2>/dev/null
	done
...
if nft list chain inet fw4 limcore_output_redir >/dev/null 2>&1; then
	nft insert rule inet fw4 limcore_output_redir ip daddr "{ $IPSET }" tcp dport 443 counter return comment "\"$CMT\"" 2>/dev/null
fi
