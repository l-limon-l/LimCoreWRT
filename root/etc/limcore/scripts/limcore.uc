...
import { mkstemp, readfile } from 'fs';
import { urldecode_params } from 'luci.http';

/* Global variables start */
export const HP_DIR = '/etc/limcore';
export const RUN_DIR = '/var/run/limcore';
...
export function getDeviceModel() {
	let m = trim(readfile('/tmp/sysinfo/model') ?? '');
	if (!m) m = trim(readfile('/proc/device-tree/model') ?? '');
	if (!m) {
		let board_str = readfile('/etc/board.json');
		if (board_str) {
			try {
				let b = json(board_str);
				if (b?.model?.name) m = b.model.name;
			} catch(e) {}
		}
	}
	if (!m) {
		let host_cmd = executeCommand('/sbin/uci -q get system.@system[0].hostname 2>/dev/null');
		m = trim(host_cmd?.stdout ?? '');
	}
	if (!m) m = 'OpenWrt';

	m = replace(m, /\0/g, '');
	m = replace(m, /^ASUS(RT-|GT-|TUF-|ROG-)/i, 'ASUS $1');
	m = replace(m, /^([A-Za-z0-9]+)(RT-|GT-|TUF-|ROG-)/, '$1 $2');
	return trim(m);
};

export function wGET(url, ua, hwid) {
	if (!url || type(url) !== 'string')
		return null;

	if (!ua)
		ua = 'Wget/1.21 (LimCore, like v2rayN)';

	let model = replace(getDeviceModel(), '"', '');
	let hwid_header = hwid ? `--header="x-hwid: ${hwid}" ` : '';
