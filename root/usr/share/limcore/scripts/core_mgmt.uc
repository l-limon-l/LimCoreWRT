...
if (action === 'info') {
	...
	if (core !== 'singbox') {
		...
		const api_url = 'https://api.github.com/repos/shtorm-7/sing-box-extended/releases/latest';
	...
	if (core !== 'singbox' || !tmp_path || !pkg_manager) {
	...
	if (core !== 'singbox') {
		...
		if (!pkg_manager) {
			...
			const pkg_name = 'sing-box-extended';
