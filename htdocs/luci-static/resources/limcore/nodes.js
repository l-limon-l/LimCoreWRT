...
function buildByeDPICard(byedpi, isMainNode) {
	...
	return E('div', { style: 'margin-bottom:12px; padding:8px 10px; border:1px solid #ddd; border-radius:4px' }, [
		...
		E('div', { style: 'margin-top:4px; font-size:0.9em; color:#666' },
			_('Local SOCKS5 proxy for DPI bypass by <a href="https://github.com/hufrea/byedpi" target="_blank">hufrea</a>.<br>Installed from <a href="https://github.com/l-limon-l/limcore-packages" target="_blank">l-limon-l/limcore-packages</a>.'))
	]);
}
...
function buildZapretCard(zapret) {
	...
	return E('div', { style: 'margin-bottom:12px; padding:8px 10px; border:1px solid #ddd; border-radius:4px' }, [
		...
		E('div', { style: 'margin-top:4px; font-size:0.9em; color:#666' },
			_('Packet-level DPI bypass (NFQUEUE) by <a href="https://github.com/bol-van/zapret2" target="_blank">bol-van</a> (nfqws2).<br>Installed from <a href="https://github.com/l-limon-l/limcore-packages" target="_blank">l-limon-l/limcore-packages</a>.'))
	]);
}
...
function buildCurlCard(curl) {
	...
	return E('div', { style: 'margin-bottom:12px; padding:8px 10px; border:1px solid #ddd; border-radius:4px' }, [
		...
		E('div', { style: 'margin-top:4px; font-size:0.9em; color:#666' },
			_('Enables real HTTP-based ByeDPI strategy testing. Required for "Test all strategies" below.'))
	]);
}
...
async function parseVpnLink(uri) {
	...
	case 'amnezia-xray': {
		...
		/* Xray network → limcore transport */
...
function parseShareLink(uri, features) {
	...
	if (uri[0] && uri[1]) {
			...
			try {
				...
				try {
					...
					if (suri.length <= 2) {
						if (suri.length === 2)
							slabel = '#' + suri[1];
						uri[1] = limcore.decodeBase64Str(suri[0]) + slabel;
					}
				} catch(e) { }
				...
				if (url.username && url.password) {
					...
					userinfo = limcore.decodeBase64Str(decodeURIComponent(url.username)).split(':');
					if (userinfo.length > 1)
						userinfo = [userinfo[0], userinfo.slice(1).join(':')]
				}

				if (!limcore.shadowsocks_encrypt_methods.includes(userinfo[0]))
					return null;
		...
		case 'vmess':
			...
			uri = JSON.parse(limcore.decodeBase64Str(uri[1]));
...
function paintDelay(section_id, delay) {
	...
	el.style.color = limcore.delayColor(delay);
}
...
function addSpeedtestButton(s, tabname, key, belongs) {
	...
	o.description = _('Measures the nodes on this tab one by one against the same Ookla Speedtest server, each through a temporary core, so the figures compare with each other.<br>About half a minute per node: two transfers over one line would measure each other. Results fill in as each node finishes.<br>Pressing it on another tab meanwhile queues those nodes. The running connection is not touched.');
}
...
function addSweepButton(s, tabname, key, belongs) {
	...
	o.description = _('Measures every node on this tab at once, through a temporary core.<br>The running connection is not touched and nothing is switched — this only shows which nodes answer and how fast.');
}
...
function renderNodeSettings(section, data, features, main_node, routing_mode) {
	...
	s.modaltitle = L.bind(limcore.loadModalTitle, this, _('Node'), _('Add a node'), data[0]);
	s.sectiontitle = L.bind(limcore.loadDefaultLabel, this, data[0]);


	o = s.option(form.Value, 'label', _('Label'));
	o.load = L.bind(limcore.loadDefaultLabel, this, data[0]);
	o.validate = L.bind(limcore.validateUniqueValue, this, data[0], 'node', 'label');
	o.modalonly = true;
	...
	o.depends('type', 'hysteria');
	o.depends('type', 'hysteria2');
	o.validate = limcore.validatePortRange;
	o.modalonly = true;
	...
	for (let i of limcore.shadowsocks_encrypt_methods)
		o.value(i);
	/* Stream ciphers */
	o.value('aes-128-ctr');
	o.value('aes-192-ctr');
	...
	o.depends('type', 'vless');
	o.depends('type', 'vmess');
	o.validate = limcore.validateUUID;
	o.modalonly = true;
	...
	o = s.option(form.TextValue, 'vless_encryption', _('Encryption'),
		_('Post-quantum VLESS encryption (ML-KEM), as the server issued it. Leave empty for a plain VLESS handshake.<br>Padding parameters Xray keeps here have no sing-box equivalent and are dropped by themselves; suite, mode, the 0-RTT flag and the key are kept.'));
	o.depends('type', 'vless');
	o.rows = 3;
	o.monospace = true;
	o.modalonly = true;
	...
	o.onchange = function(ev, section_id, value) {
		let desc = this.map.findElement('id', 'cbid.limcore.%s.transport'.format(section_id)).nextElementSibling;
		if (value === 'http')
		...
		let tls = this.map.findElement('id', 'cbid.limcore.%s.tls'.format(section_id)).firstElementChild;
		if ((value === 'http' && tls.checked) || (value === 'grpc' && !features.with_grpc)) {
			this.map.findElement('id', 'cbid.limcore.%s.http_idle_timeout'.format(section_id)).nextElementSibling.innerHTML =
			...
			this.map.findElement('id', 'cbid.limcore.%s.http_ping_timeout'.format(section_id)).nextElementSibling.innerHTML =
	...
	o.depends('type', 'wireguard');
	o.depends('type', 'amneziawg');
	o.validate = L.bind(limcore.validateBase64Key, this, 44);
	o.rmempty = false;
	o.modalonly = true;
	...
	o.value('', _('default'));
	for (let i of limcore.tls_versions)
		o.value(i);
	o.depends('tls', '1');
	o.modalonly = true;
	...
	o.value('/etc/limcore/certs/client_ca.pem');
	o.depends('tls_self_sign', '1');
	o.validate = limcore.validateCertificatePath;
	o.rmempty = false;
	o.modalonly = true;
	...
	o.depends({'tls_self_sign': '1', 'tls_cert_path': '/etc/limcore/certs/client_ca.pem'});
	o.onclick = L.bind(limcore.uploadCertificate, this, _('certificate'), 'client_ca');
	o.modalonly = true;
	...
	o.value('/etc/limcore/certs/client_ech_conf.pem');
	o.depends('tls_ech', '1');
	o.modalonly = true;
	...
	o.inputstyle = 'action';
	o.inputtitle = _('Upload...');
	o.depends({'tls_ech': '1', 'tls_ech_config_path': '/etc/limcore/certs/client_ech_conf.pem'});
	o.onclick = L.bind(limcore.uploadCertificate, this, _('ECH config'), 'client_ech_conf');
	o.modalonly = true;
	...
	if (features.with_utls) {
		...
		o.validate = function(section_id, value) {
			if (section_id) {
				let tls_reality = this.map.findElement('id', 'cbid.limcore.%s.tls_reality'.format(section_id)).firstElementChild;
...
/* The node form backs two menu pages: the nodes themselves, and the DPI bypass tools
 * whose strategies live in the same section. See renderPage() in limcore.js. */
const DPI_TABS = [ 'byedpi', 'zapret' ];
...
const NodesView = view.extend({
	load() {
		return Promise.all([
			...
			L.resolveDefault(fs.read('/etc/limcore/zapret_candidates.json'), null),
			(this.page === 'dpi') ? L.resolveDefault(callByeDPIStatus(), {}) : {},
			(this.page === 'dpi') ? L.resolveDefault(callCurlStatus(), {}) : {},
			(this.page === 'dpi') ? L.resolveDefault(callZapretStatus(), {}) : {}
		]);
	},
	...
	render(data) {
		...
		m = new form.Map('limcore', (this.page === 'dpi') ? _('DPI bypass') : _('Nodes'));

		s = m.section(form.NamedSection, 'subscription', 'limcore');

		/* Node settings start */
		/* User nodes start */
		/* The nodes added here by hand, and only those. Subscription nodes belong to the
		 * provider — every update rewrites them, so an edit made here would not last —
		 * and they are picked from the overview, grouped by subscription. */
		s.tab('node', _('Nodes'));
		/* Own = not from a subscription that still exists, so a node left behind by a
		 * removed one stays reachable here instead of vanishing from every page. */
		const liveSubs = (uci.get(data[0], 'subscription', 'subscription_url') || [])
			.map((u) => limcore.calcStringMD5(u.replace(/#.*$/, '')));
		const ownNode = (section_id) => !liveSubs.includes(uci.get(data[0], section_id, 'grouphash'));
		...
		ss.handleLinkImport = function() {
			...
			ui.showModal(_('Import share links'), [
				E('p', _('Supported: Amnezia (vpn://), Hysteria, Mieru, NaïveProxy (naive://), Shadowsocks (ss://), SSH, Trojan, v2rayN (VMess), WireGuard and VLESS (vless://).')),
				textarea.render(),
				E('div', { class: 'right' }, [
					E('button', {
						class: 'btn',
							...
							return Promise.all(vpnLinks.map(l => parseVpnLink(l).catch(() => null)))
								.then(vpnConfigs => {
									...
									configs.forEach((config) => {
										...
										let nameHash = limcore.calcStringMD5(config.label);
		...
		/* Every hour by default, as Happ does: an update that finds nothing new leaves the
		 * core running, so checking often costs nothing but the download. */
		o = s.taboption('subscription', form.ListValue, 'auto_update_interval', _('Update interval'),
			_('The connection is not interrupted when the provider has changed nothing; when it has, LimCore restarts for a few seconds.'));
		o.value('1', _('Every hour'));
		for (const h of [ 2, 3, 4, 6, 12 ])
			o.value(String(h), _('Every %d hours').format(h));
		...
		for (let i = 0; i < 24; i++)
			o.value(i, i + ':00');
		o.default = '2';
		o.depends('auto_update_interval', '24');
		/* Kept while hidden, so going back to once a day finds the hour chosen before. */
		o.retain = true;
		...
		o.placeholder = 'Wget/1.21 (LimCore, like v2rayN)';

		o = s.taboption('subscription', form.Value, 'hwid', _('HWID'));
		o.description = _('Unique Hardware ID sent to the VPN provider for subscription updates. Generated from router hardware. Copy this value and send it to your VPN provider bot.');
		o.rmempty = false;
		o.load = function(section_id) {
			return uci.get('limcore', section_id, 'hwid') || uci.get('limcore', 'config', 'hwid') || '';
		};
		...
		o.onclick = function() {
			...
			return fs.exec_direct('/etc/limcore/scripts/update_subscriptions.uc').then((out) => {
				if (!String(out).includes('LIMCORE_SUB_OK'))
					throw new Error(_('the update did not finish, the LimCore log has the details'));
				return location.reload();
			}).catch((err) => {
				ui.hideModal();
				ui.addNotification(null, E('p', _('An error occurred during updating subscriptions: %s').format(err?.message || err)));
		...
		o.inputtitle = function() {
			/* Counted as the servers the overview shows: the hidden members of a
			 * provider's group go with it, but counting them made "60 nodes" out of 24. */
			let subnodes = [];
			uci.sections(data[0], 'node', (res) => {
				if (res.grouphash && !res.member_of)
		...
		/* Ping settings start */
		/* How every delay on these pages and the overview is taken - the choices Happ offers.
		 * Read by rpcd on each check, so a saved change counts from the next press and the
		 * core is not restarted for it. */
		/* Its own title: the plain "Ping" is taken by LuCI's base catalogue, as "Пинг-запрос". */
		s.tab('ping', _('Ping settings'));
		let pingSV = s.taboption('ping', form.SectionValue, '_ping', form.NamedSection, 'config', 'limcore');
		ss = pingSV.subsection;
		ss.anonymous = true;
		ss.addremove = false;

		o = ss.option(form.ListValue, 'ping_type', _('Ping type'),
			_('<b>Through the node</b> — a request goes through the node to the test URL, the way a browser would feel it.<br><b>TCP</b> and <b>ICMP</b> — only the way from the router to the node\'s server, without the node\'s own handshake.<br>Nodes over UDP (Hysteria, TUIC, WireGuard) are always measured through the node.'));
		o.value('get', _('Through the node, GET'));
		o.value('head', _('Through the node, HEAD'));
		o.value('tcp', 'TCP');
		o.value('icmp', 'ICMP');
		o.default = 'get';
		o.rmempty = false;

		o = ss.option(form.ListValue, 'ping_mode', _('Mode (through the node)'),
			_('Two requests are sent, the better one is shown.<br><b>default</b> — each as in a fresh session: a new connection, and the node\'s server looked up again.<br><b>double</b> — each with a connection of its own, the handshake counts in the figure.<br><b>keepalive</b> — both over one connection, the second shows the node\'s answer alone.'));
		o.value('default', 'default');
		o.value('double', 'double');
		o.value('keepalive', 'keepalive');
		o.default = 'default';
		o.rmempty = false;
		/* Hidden for TCP and ICMP, but kept: LuCI drops a hidden option on save otherwise. */
		o.retain = true;
		o.depends('ping_type', 'get');
		o.depends('ping_type', 'head');

		o = ss.option(form.Value, 'ping_url', _('Test URL (through the node)'),
			_('The one Happ uses. Change it only if you need another; it should answer quickly and with no body.'));
		o.placeholder = 'https://www.gstatic.com/generate_204';
		o.default = 'https://www.gstatic.com/generate_204';
		o.rmempty = false;
		o.retain = true;
		/* Wide enough to read the whole address. The theme's field column is what holds it to
		 * about 230px, so that is widened, for this row only, and the input fills it. */
		o.render = function(/* ... */) {
			return form.Value.prototype.render.apply(this, arguments).then((row) => {
				const field = row?.querySelector?.('.cbi-value-field');
				if (field) {
					field.style.width = '26em';
					field.style.maxWidth = '100%';
					field.style.flex = '0 1 26em';
					field.querySelectorAll('input').forEach((el) => {
						el.style.width = '100%';
						if (el.parentElement !== field) el.parentElement.style.width = '100%';
					});
				}
				return row;
			});
		...
		o.validate = function(section_id, value) {
			if (!value)
				return true;
			return /^https?:\/\/[^\s'"`$\\]+$/.test(value) ? true : _('Expecting: %s').format(_('an http:// or https:// address'));
		};
		/* Ping settings end */

		/* ByeDPI settings start */
		s.tab('byedpi', _('ByeDPI'));
		let byedpiSV = s.taboption('byedpi', form.SectionValue, '_byedpi', form.NamedSection, 'config', 'limcore');
		ss = byedpiSV.subsection;
		ss.anonymous = true;
		ss.addremove = false;

		o = ss.option(form.DummyValue, '_byedpi_card');
		o.default = buildByeDPICard(data[3], main_node === 'byedpi-out');

		o = ss.option(form.Flag, 'byedpi_enabled', _('Enable ByeDPI'),
			_('A free way to unblock throttled sites (YouTube and the like) without a VPN subscription.<br>It confuses the ISP\'s traffic analysis; traffic is <b>not</b> encrypted or hidden, and the ISP still sees which sites you visit.<br>Results depend on the ISP — a strategy may need picking on the ByeDPI tab. Installed by itself the first time it is turned on.'));
		o.default = o.disabled;
		o.rmempty = false;
		(function(opt) {
				...
				return Promise.resolve(_super(section_id, option_index, cfgvalue)).then(function(node) {
					node.querySelector('input').addEventListener('change', async function(ev) {
						...
						if (!status.pkg_manager) {
							ui.addNotification(null, E('p', _('No package manager found. Install ciadpi manually.')), 'error');
		...
		o = ss.option(form.DummyValue, '_curl_card', _('ByeDPI strategy tester'));
		o.default = buildCurlCard(data[4]);
		...
		(function() {
			...
			o.render = function(option_index, section_id) {
				const cur = (uci.get('limcore', 'config', 'byedpi_cmd_opts') || '').trim();
				let curIdx = -1;
				...
				return E('div', { 'class': 'cbi-value' }, [
					...
					E('div', { 'class': 'cbi-value-field' }, [
						...
						E('div', { 'class': 'cbi-value-description' },
							_('Grouped by technique.<br>Based on <a href="https://github.com/hufrea/byedpi" target="_blank">hufrea/byedpi</a> and community testing; see also <a href="https://github.com/fatyzzz/Byedpi-Setup" target="_blank">fatyzzz/Byedpi-Setup</a>.'))
					])
				]);
			};
			o.write = function() {};
			...
			o = ss.option(form.DummyValue, '_byedpi_tester',
				...
				_('Starts ByeDPI with the current options on a temporary port and checks 4 sites: <b>YouTube</b> and <b>Telegram</b> (far servers), <b>Discord</b> and <b>Speedtest.net</b> (near Cloudflare/Ookla edges).<br>Passing the far ones but failing the near ones means the strategy depends on the destination (typical of a fixed <code>--fake --ttl</code>).<br>Needs curl; without it only the start of ByeDPI is confirmed.'));
			o.render = function(option_index, section_id) {
				...
				return E('div', { 'class': 'cbi-value' }, [
					...
					E('div',   { 'class': 'cbi-value-field' }, [ btn, msgEl, E('div', { 'class': 'cbi-value-description' }, this.description) ])
				]);
			};
			o.write = function() {};
			...
			o.render = function(option_index, section_id) {
				...
				const hintEl = E('div', { style: 'font-size:0.85em; color:#666; margin-bottom:6px' },
					_('Checks every preset on 4 sites (YouTube video CDN, Telegram, Discord, Speedtest). About 12 minutes; can be stopped any time.<br>● the TLS handshake got through, ○ blocked. Hover a dot for the site and the result.'));

				L.resolveDefault(callCurlStatus(), {}).then(function(status) {
					if (status.installed) {
						btn.disabled = false;
					} else {
						hintEl.textContent = _('Requires curl. Install it above first.');
		...
		o = ss.option(form.Flag, 'byedpi_block_quic',
			_('Block QUIC (UDP port 443)'),
			_('Drops all outgoing UDP to port 443, so browsers and apps fall back to TCP/TLS, where ByeDPI works.<br>Side effects: services that need QUIC may break, and every device on the network is affected.<br>Turn on if the strategy works but some sites still do not load.'));
		o.default = o.disabled;
		o.rmempty = false;
		/* ByeDPI settings end */
		...
		let zapretSV = s.taboption('zapret', form.SectionValue, '_zapret', form.NamedSection, 'config', 'limcore');
		ss = zapretSV.subsection;
		ss.anonymous = true;
		ss.addremove = false;

		o = ss.option(form.DummyValue, '_zapret_card');
		o.default = buildZapretCard(data[5]);

		o = ss.option(form.Flag, 'zapret_enabled', _('Enable Zapret'),
			_('An alternative to ByeDPI: another free way to unblock throttled or blocked sites (YouTube, Discord…) without a VPN subscription.<br>Unlike ByeDPI it also handles QUIC.<br>A working strategy depends on the ISP and has to be picked. Installed by itself the first time it is turned on.'));
		o.default = o.disabled;
		o.rmempty = false;
		(function(opt) {
			const _super = opt.renderWidget.bind(opt);
					...
					node.querySelector('input').addEventListener('change', async function(ev) {
						...
						if (status.installed) {
							...
							if (status.kmod_ok === false) {
								ui.addNotification(null, E('p', _('Zapret is installed, but the NFQUEUE kernel module (kmod-nft-queue) is missing.<br>Turning it on now could break the firewall. Reinstall Zapret or install kmod-nft-queue first.')), 'error');
								ev.target.checked = false;
							}
							return;
						}
						...
						if (!status.pkg_manager) {
							ui.addNotification(null, E('p', _('No package manager found. Install zapret2 manually.')), 'error');
							ev.target.checked = false;
							return;
						}
		...
		(function() {
			 ...
			 * /etc/limcore/zapret_candidates.json (loaded in load()). Each entry =
			...
			o.render = function(option_index, section_id) {
				const cur = (uci.get('limcore', 'config', 'zapret_cmd_opts') || '').trim();
				let curIdx = -1;
				...
				return E('div', { 'class': 'cbi-value' }, [
					...
					E('div', { 'class': 'cbi-value-field' }, [ sel,
						E('div', { 'class': 'cbi-value-description' }, _('Fills in the strategy below. Use the test to find one that works with your ISP.<br>Based on <a href="https://github.com/bol-van/zapret2" target="_blank">bol-van/zapret2</a> (nfqws2/blockcheck2) and <a href="https://github.com/flowseal/zapret-discord-youtube" target="_blank">flowseal/zapret-discord-youtube</a> (MIT). Installed from <a href="https://github.com/l-limon-l/limcore-packages" target="_blank">l-limon-l/limcore-packages</a>.')) ])
				]);
			};
			o.write = function() {};

			o = ss.option(form.Value, 'zapret_cmd_opts', _('Desync strategy'),
				_('nfqws2 options for every flow sent to Zapret. Pick a preset above or edit freely.<br>No host lists: the routing rules already decide what comes here.<br>The <code>--qnum</code>/<code>--user</code>/<code>--fwmark</code>/<code>--lua-init</code> arguments are added automatically.'));
			...
			o = ss.option(form.DummyValue, '_zapret_tester', _('Test current strategy'),
				_('Runs the current strategy on a temporary NFQUEUE against 4 sites — <b>YouTube</b>, <b>Telegram</b>, <b>Discord</b>, <b>Speedtest.net</b> — and checks each TLS handshake.<br>Your connection is not touched. A site that fails here is one the strategy breaks.'));
			o.render = function(option_index, section_id) {
				const msgEl = E('span', { style: 'margin-left:8px; font-size:0.9em; color:gray' }, '');
				const btn = E('button', {
					'class': 'btn cbi-button cbi-button-action',
					'click': ui.createHandlerFn(this, async () => {
						...
						let opts = (w && w.getValue() != null) ? String(w.getValue())
							: (uci.get('limcore', 'config', 'zapret_cmd_opts') || '');
						opts = opts.trim();
						btn.disabled = true;
						msgEl.style.color = 'gray';
						msgEl.textContent = _('Testing...');
			...
			o = ss.option(form.DummyValue, '_zapret_fulltest', _('Full strategy test'),
				_('Tries every strategy in the list in turn on a temporary NFQUEUE and shows which pass. Your connection is not touched.<br>Stay on this page while it runs — leaving stops it.<br>Press <b>Apply</b> next to a passing strategy to pick it, then Save &amp; Apply.'));
			o.render = function(option_index, section_id) {
				...
				return E('div', { 'class': 'cbi-value' }, [
					...
					E('div', { 'class': 'cbi-value-field' }, [ runBtn, stopBtn, E('div', { 'class': 'cbi-value-description' }, this.description), prog, list ])
				]);
			};
			o.write = function() {};
			...
			o = ss.option(form.Flag, 'zapret_voice', _('Discord calls via Zapret'),
				_('Send Discord voice UDP ports (19294–19344, 50000–65530) through Zapret.<br>Discord voice has no domain to match, so only the ports can pick it. Leave off if your VPN already carries calls.'));
			o.default = '0';
			o.rmempty = false;
		})();
		...
		const onPage = (this.page === 'dpi') ? DPI_TABS
			: s.tab_names.filter((t) => !DPI_TABS.includes(t));
		return limcore.renderPage(m, s, onPage);
	}
});

return baseclass.extend({
	page(page) {
		return NodesView.extend({ page: page });
	}
});
