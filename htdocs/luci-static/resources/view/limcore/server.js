...
function getServiceStatus() {
	return L.resolveDefault(callServiceList('limcore'), {}).then((res) => {
		let isRunning = false;
		try {
			isRunning = res['limcore']['instances']['limcore']['running'];
		} catch (e) { }
		return isRunning;
	});
}
...
function renderStatus(isRunning, features) {
	...
	let renderHTML;
	if (isRunning)
		renderHTML = spanTemp.format('green', _('LimCore Server'), coreStr, _('RUNNING'));
	else
		renderHTML = spanTemp.format('red', _('LimCore Server'), coreStr, _('NOT RUNNING'));
