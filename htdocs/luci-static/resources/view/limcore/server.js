...
const CBIGenValue = form.Value.extend({
	...
	renderWidget(/* ... */) {
		...
		(node.querySelector('.control-group') || node).appendChild(E('button', {
			...
			title: _('Generate'),
			click: ui.createHandlerFn(this, handleGenKey, this.lc_options || this.option)
		}, [ _('Generate') ]));
	...
	if (typeof option === 'object') {
		...
		switch (required_method) {
			case 'none':
				...
				break;
			case 'uuid':
				password = limcore.generateRand('uuid');
				break;
			default:
				password = limcore.generateRand('hex', 16);
				break;
		}
		/* AEAD */
		((length) => {
			if (length && length > 0)
				password = limcore.generateRand('base64', length);
		})(limcore.shadowsocks_encrypt_length[required_method]);
...
return view.extend({
	...
	render(data) {
		...
		m = new form.Map('limcore', _('LimCore Server'),
			_('The modern multi-core proxy platform. Created by l_limon_l.'));

		s = m.section(form.NamedSection, 'server', 'limcore', _('Global settings'));
		...
		s.modaltitle = L.bind(limcore.loadModalTitle, this, _('Server'), _('Add a server'), data[0]);
		s.sectiontitle = L.bind(limcore.loadDefaultLabel, this, data[0]);
		s.renderSectionAdd = L.bind(limcore.renderSectionAdd, this, s);

		o = s.option(form.Value, 'label', _('Label'));
		o.load = L.bind(limcore.loadDefaultLabel, this, data[0]);
		o.validate = L.bind(limcore.validateUniqueValue, this, data[0], 'server', 'label');
		o.rmempty = false;
		o.modalonly = true;
		...
		o.datatype = 'port';
		o.validate = L.bind(limcore.validateUniqueValue, this, data[0], 'server', 'port');
				...
				if (required_type.includes(type)) {
					if (type === 'shadowsocks') {
						...
						if (encmode === 'none')
							...
							return limcore.validateBase64Key(24, section_id, value);
						else if (['2022-blake3-aes-256-gcm', '2022-blake3-chacha20-poly1305'].includes(encmode))
							return limcore.validateBase64Key(44, section_id, value);
					}
		...
		*/
		...
		o.onchange = function(ev, section_id, value) {
			...
			let tls_element = this.map.findElement('id', 'cbid.limcore.%s.tls'.format(section_id)).firstElementChild;
		...
		o.value('/etc/limcore/certs/server_publickey.pem');
		...
		o.depends({'tls': '1', 'tls_cert_path': '/etc/limcore/certs/server_publickey.pem'});
		o.onclick = L.bind(limcore.uploadCertificate, this, _('certificate'), 'server_publickey');
		o.modalonly = true;

		o = s.option(form.Value, 'tls_key_path', _('Key path'),
			_('The server private key, in PEM format.'));
		o.value('/etc/limcore/certs/server_privatekey.pem');
		o.depends({'tls': '1', 'tls_acme': '0', 'tls_reality': '0'});
		o.depends({'tls': '1', 'tls_acme': '0', 'tls_reality': null});
		o.depends({'tls': '1', 'tls_acme': null, 'tls_reality': '0'});
		o.depends({'tls': '1', 'tls_acme': null, 'tls_reality': null});
		...
		o.inputstyle = 'action';
		o.inputtitle = _('Upload...');
		o.depends({'tls': '1', 'tls_key_path': '/etc/limcore/certs/server_privatekey.pem'});
		o.onclick = L.bind(limcore.uploadCertificate, this, _('private key'), 'server_privatekey');
		o.modalonly = true;
			...
			node.appendChild(E('div',  { 'class': 'control-group' }, [
				...
				E('button', {
					...
					click: ui.createHandlerFn(this, () => {
						this.lc_options.params = document.getElementById(cbid).value;

						return handleGenKey.call(this, this.lc_options);
					})
				}, [ _('Generate') ])
			]));
