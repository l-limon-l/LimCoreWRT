...
.diag-label { min-width:11em; color:#888; font-size:.9em; flex-shrink:0; }\
.diag-ok   { color: #2a2; font-weight: bold; }\
.diag-fail { color: #c33; font-weight: bold; }\
.diag-warn { color: #c80; font-weight: bold; }\
.diag-gray { color: #888; }\
.diag-pre { font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace; font-size: .85em; line-height: 1.4; background: #18181b; color: #e4e4e7; border: 1px solid #27272a; padding: .6em .9em; white-space: pre-wrap; word-break: break-all; border-radius: 6px; margin: .4em 0 0; max-height: 16em; overflow-y: auto; }\
.diag-input { padding:.25em .4em; border:1px solid #333; border-radius:3px; font-size:.9em; width:14em; background:#18181b; color:#e4e4e7; }\
.diag-btn { margin-right:.4em; }\
.diag-port-line { font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace; font-size: .85em; margin: .1em 0; }\
#diag-report-area { width: 100%; height: 22em; font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace; font-size: .85em; line-height: 1.4; white-space: pre; overflow: auto; background: #18181b; color: #e4e4e7; border: 1px solid #27272a; border-radius: 6px; padding: .6em; box-sizing: border-box; }\
';
...
function buildConnectivitySection(view, coreType) {
		...
		poll.add(L.bind(function() {
			return L.resolveDefault(callActiveNode(tag), {}).then(function(ret) {
				if (ret && !ret.error && ret.node) {
					...
					dom.content(resultEl, E('strong', {
							'class': cls,
							/* The colour from the helper as well: diag-warn is this page's orange
							   for warnings, and a delay has to wear the yellow it wears in the
							   node grid. */
							'style': 'color:' + limcore.delayColor(ret.delay)
						}, resolveTag(ret.node) + type + dStr));
				} else {
	...
	const udpNode   = uci.get('limcore', 'config', 'main_udp_node');
	const proxyMode = uci.get('limcore', 'config', 'proxy_mode');
...
function buildNftSection(view) {
	...
	function run() {
		...
		return L.resolveDefault(callNftables(), {}).then(function(ret) {
			...
			dom.content(resultsEl, [
				row(_('LimCore chains'), statusBadge(ret.nft_present,
				...
				ret.udp_tproxy ? E('div', {}, [
					E('div', { 'class': 'diag-label' }, _('UDP TPROXY counters (UDP into LimCore)')),
					pre(ret.udp_tproxy)
				]) : null,
...
function buildReportSection(view) {
	...
	function download() {
		...
		const a    = E('a', { href: url, download: 'limcore-diagnostics-' + stamp + '.txt' });
...
return view.extend({
	load: function() {
		return Promise.all([
			L.resolveDefault(limcore.getBuiltinFeatures(), {}),
			uci.load('limcore')
		]);
	},
