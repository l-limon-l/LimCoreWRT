...
# The version is set by hand in the Makefile: 2.0, then 2.0.1, 2.0.2 for fixes and 2.1
# for larger updates. The release tag is "v" + this version.
...
default_prerm' > "$TEMP_DIR/pre-deinstall"
	...
	apk mkpkg \
		...
		--info "description:LimCore - modern multi-core proxy platform" \
		--info "arch:noarch" \
	...
	if [ "$LEGACY" == "legacy" ]; then
		...
		cat > /tmp/lc_legacy_patch.py << 'PYEOF'
import sys
path = sys.argv[1]
with open(path) as f:
    c = f.read()
...
md5_fn = """
function md5(s) {
\tconst tmp = '/tmp/.lc_md5tmp';
\tconst f = open(tmp, 'w');
\tif (!f) return '';
\tf.write(s);
\tf.close();
\tconst fd = popen('md5sum < /tmp/.lc_md5tmp');
\tif (!fd) return '';
...
with open(path, 'w') as f:
    f.write(c)
PYEOF
		python3 /tmp/lc_legacy_patch.py "$SUBSC_UC"
		rm -f /tmp/lc_legacy_patch.py
	...
	cat > "$TEMP_PKG_DIR/CONTROL/control" <<-EOF
		...
		Version: $PKG_VERSION
		Depends: $IPK_DEPS
		Conflicts: luci-app-re-homeproxy luci-app-homeproxy luci-app-homeproxy-hiddify
		Replaces: luci-app-re-homeproxy luci-app-homeproxy luci-app-homeproxy-hiddify
		...
		Description:  LimCore - modern multi-core proxy platform
	EOF
...
if [ "$PKG_MGR" == "apk" ]; then
	...
	cat > "$I18N_TEMP_PKG_DIR/CONTROL/control" <<-EOF
		...
		Version: $PKG_VERSION
		Depends: $PKG_NAME
		Conflicts: luci-i18n-homeproxy-ru
		Replaces: luci-i18n-homeproxy-ru
		Source: https://github.com/l-limon-l/LimCoreWRT
...
if [ "$PKG_MGR" == "apk" ]; then
	...
	apk mkpkg \
		...
		--info "maintainer:l_limon_l" \
		--info "depends:$PKG_NAME" \
		--info "provides:luci-i18n-homeproxy-zh-cn" \
	...
	cat > "$I18N_ZH_TEMP_PKG_DIR/CONTROL/control" <<-EOF
		...
		Version: $PKG_VERSION
		Depends: $PKG_NAME
		Conflicts: luci-i18n-homeproxy-zh-cn
...
if [ "$PKG_MGR" == "apk" ]; then
	...
	apk mkpkg \
		...
		--info "maintainer:l_limon_l" \
		--info "depends:$PKG_NAME" \
		--info "provides:luci-i18n-homeproxy-fa" \
		--info "replaces:luci-i18n-homeproxy-fa" \
		${APK_SIGN_KEY:+--sign-key "$APK_SIGN_KEY"} \
	...
	cat > "$I18N_FA_TEMP_PKG_DIR/CONTROL/control" <<-EOF
		...
		Version: $PKG_VERSION
		Depends: $PKG_NAME
		Conflicts: luci-i18n-homeproxy-fa
		Replaces: luci-i18n-homeproxy-fa
		Source: https://github.com/l-limon-l/LimCoreWRT
