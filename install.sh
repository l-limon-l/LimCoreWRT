...
DO_ZAPRET=0
if [ "$AUTO" = 1 ]; then
	[ "$(uci -q get homeproxy.config.zapret_enabled)" = "1" ] || [ -x "/opt/zapret2/nfq2/nfqws2" ] && DO_ZAPRET=1
else
...
DO_BYEDPI=0
if [ "$AUTO" = 1 ]; then
	[ "$(uci -q get homeproxy.config.byedpi_enabled)" = "1" ] || [ -x "/usr/bin/ciadpi" ] && DO_BYEDPI=1
else
