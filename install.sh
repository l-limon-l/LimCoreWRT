...
if [ "$PM" = apk ]; then
	...
	# --force-downgrade: v2 нумеруется 2.0, 2.0.1, 2.1…, а старые сборки — датой (2026.09.26-r127),
	# и без флага opkg счёл бы переход на v2 откатом и молча отказался ставить.
	opkg update >/dev/null 2>&1; opkg install --force-downgrade /tmp/app.$EXT || die "opkg install завершился ошибкой."
fi
rm -f /tmp/app.$EXT
...
case "$REPLY" in
	...
	*)
		...
		if [ -n "$LURL" ] && dl "$LURL" /tmp/i18n.$EXT; then
			...
			else opkg install --force-downgrade /tmp/i18n.$EXT; fi
...
# Шаг спрашивается, а не выполняется молча: ядро — самый крупный пакет установки
# (~75 МБ), и бывает, что ставить его сейчас не нужно (обновление приложения) или
# некуда (мало флеша).
#
# Отказ означает, что работать не будет НИЧЕГО, и обещать иное нельзя: Zapret и
# ByeDPI — это outbound'ы самого ядра (zapret-out ставит routing_mark, который nft
# затем уводит в NFQUEUE; byedpi-out — SOCKS на 127.0.0.1:5335). Трафик доводит до
# них sing-box, поэтому без него оба молчат, даже будучи установленными.
# По умолчанию «да»: молчаливый Enter не должен оставлять человека ни с чем.
DO_CORE=1
if [ "$AUTO" != 1 ]; then
	if [ "$CORE_PRESENT" = 1 ]; then
		ask "[2/5] Ядро sing-box уже установлено. Обновить его? [Y/n] (по умолчанию Y):"
	else
		ask "[2/5] Установить ядро sing-box-extended (~75 МБ)? Без него LimCore не заработает — Zapret и ByeDPI тоже идут через ядро. [Y/n] (по умолчанию Y):"
	fi
...
[ "$CORE_OK" = 1 ] && ok "  ядро установлено: $(/usr/bin/sing-box version 2>/dev/null | head -1)"
	...
	if [ "$CORE_PRESENT" = 1 ]; then
		...
		warn "  Без ядра LimCore не работает целиком: Zapret и ByeDPI — это выходы самого"
		warn "  ядра, трафик доводит до них оно, поэтому в одиночку они ничего не сделают."
		info "  Поставьте ядро позже на вкладке «Ядро и службы» — приложение уже установлено."
		CORE_SKIPPED=1
	fi
fi
	...
	if [ -z "$ZP" ] || jerr "$ZP"; then warn "  не удалось подготовить Zapret — пропускаю. ($(jget "$ZP" error))"
		...
		if [ -n "$ZURL" ] && dl "$ZURL" "$ZTMP"; then
			...
			if jtrue "$RES"; then
				...
				# An update (--auto) leaves the switch where the user put it: it used to turn
				# Zapret back on after every update, over the user having turned it off.
				if [ "$AUTO" = 1 ]; then
					:
				elif jfalse "$ZST" kmod_ok; then
	...
	if [ -z "$BP" ] || jerr "$BP"; then warn "  не удалось подготовить ByeDPI — пропускаю. ($(jget "$BP" error))"
		...
		if [ -n "$BURL" ] && dl "$BURL" "$BTMP"; then
			...
			if jtrue "$RES"; then
				...
				# As with Zapret: an update keeps the switch as it was.
				if [ "$AUTO" != 1 ]; then
...
if [ "$CORE_SKIPPED" = 1 ]; then
	...
	warn "Ядро не установлено — без него не работает ни прокси, ни Zapret, ни ByeDPI."
